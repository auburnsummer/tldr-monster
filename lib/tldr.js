...
const getFiles = async (event) => {
    ...
    const cacheString = "https://api.github.com/graphql/1";
    ...
    if (!response) {
      ...
      response.headers.set('Cache-Control', `max-age=${constants.CACHE_TIME}`);
      // store it
      event.waitUntil(cache.put(cacheString, response.clone()));
    }
